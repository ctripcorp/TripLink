...
class CardCloseTest extends TestCase
{
    ...
    public function test()
    {
        ...
        $queryCloseRequest = new QueryCloseResultRequest();

        $queryCloseRequest->setRequestId("close20210728224700001");
        $queryCloseRequest->setRequestTime("20210728230050");
        $queryCloseRequest->setMerchantName("WANWEI");
        $queryCloseRequest->setOperator("max");

        $cardOperation = new CardOperation($URL, $PRIVATE_KEY, $RSA_KEY);
        $cardOperation->queryCloseCardResult($queryCloseRequest);
    }

<?php
require_once __DIR__ . '/../vendor/autoload.php';

use PHPUnit\Framework\TestCase;

require_once(__DIR__ . '/../src/triplink/vcc/card/client/CardOperation.php');
require_once(__DIR__ . '/../src/triplink/vcc/card/client/operate/CreateCardRequest.php');
require_once(__DIR__ . '/../src/triplink/vcc/card/client/operate/CloseCardRequest.php');

require_once(__DIR__ . '/../src/triplink/vcc/card/client/operate/CreateCardInfo.php');
require_once(__DIR__ . '/../src/triplink/vcc/card/client/operate/UpdateCardRequest.php');
require_once(__DIR__ . '/../src/triplink/vcc/card/client/operate/UpdateCardInfo.php');

require_once(__DIR__ . '/../src/triplink/vcc/card/client/query/QueryCloseResultRequest.php');
require_once(__DIR__ . '/../src/triplink/vcc/card/client/query/QueryCloseResultResponse.php');

require_once(__DIR__ . '/../src/triplink/vcc/card/client/query/QueryAuthTransInfoRequest.php');
require_once(__DIR__ . '/../src/triplink/vcc/card/client/query/QueryAuthTransInfoResponse.php');


use triplink\vcc\card\client\CardOperation;
use triplink\vcc\card\client\operate\CreateCardRequest;
use triplink\vcc\card\client\operate\UpdateCardRequest;
use triplink\vcc\card\client\operate\CloseCardRequest;
use triplink\vcc\card\client\operate\CreateCardInfo;
use triplink\vcc\card\client\operate\UpdateCardInfo;

use triplink\vcc\card\client\query\QueryCloseResultRequest;
use triplink\vcc\card\client\query\QueryCloseResultResponse;

use triplink\vcc\card\client\query\QueryAuthTransInfoRequest;
use triplink\vcc\card\client\query\QueryAuthTransInfoResponse;



class AuthQueryTest extends TestCase
{


    public function test()
    {
        $URL = "https://openpci.fws.ctripqa.com/restful/soa2/18375/json";

        $PRIVATE_KEY = "";

        $RSA_KEY = "Wa8Nw*Ei";

        $queryAuthTransRequest = new QueryAuthTransInfoRequest();

        $queryAuthTransRequest->setRequestId("auth20210728224700002");
        $queryAuthTransRequest->setRequestTime("20210728230050");
        $queryAuthTransRequest->setMerchantName("WANWEI");
        $queryAuthTransRequest->setChannelType("WANWEI_CFNC_VCC");
        $queryAuthTransRequest->setCardLogId("db8b3994d938d9d82d180041c2f6dd180a8d88ec7c8b04f9c417a3971e273225");
        $queryAuthTransRequest->setOperator("max");

        $cardOperation = new CardOperation($URL, $PRIVATE_KEY, $RSA_KEY);
        $cardOperation->queryAuthTransInfo($queryAuthTransRequest);
    }

}
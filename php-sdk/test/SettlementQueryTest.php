...
require_once(__DIR__ . '/../src/triplink/vcc/card/client/query/QueryCloseResultRequest.php');
require_once(__DIR__ . '/../src/triplink/vcc/card/client/query/QueryCloseResultResponse.php');

require_once(__DIR__ . '/../src/triplink/vcc/card/client/query/QuerySettlementTransInfoRequest.php');
require_once(__DIR__ . '/../src/triplink/vcc/card/client/query/QuerySettlementTransInfoResponse.php');
...
use triplink\vcc\card\client\query\QueryCloseResultRequest;
use triplink\vcc\card\client\query\QueryCloseResultResponse;

use triplink\vcc\card\client\query\QueryAuthTransInfoRequest;
use triplink\vcc\card\client\query\QueryAuthTransInfoResponse;

use triplink\vcc\card\client\query\QuerySettlementTransInfoRequest;
use triplink\vcc\card\client\query\QuerySettlementTransInfoResponse;



class SettlementQueryTest extends TestCase
{
    ...
    public function test()
    {
        ...
        $querySettlementTransRequest = new QuerySettlementTransInfoRequest();

        $querySettlementTransRequest->setRequestId("auth20210728224700002");
        $querySettlementTransRequest->setRequestTime("20210728230050");
        $querySettlementTransRequest->setMerchantName("WANWEI");
        $querySettlementTransRequest->setChannelType("WANWEI_CFNC_VCC");
        $querySettlementTransRequest->setCardLogId("f5c77fabe03d174ab7b738d782f3e18b5ed06862aacac4a197e273b81f9657fd");
        $querySettlementTransRequest->setOperator("max");

        $cardOperation = new CardOperation($URL, $PRIVATE_KEY, $RSA_KEY);
        $cardOperation->querySettlementTransInfo($querySettlementTransRequest);
    }
